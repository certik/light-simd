...
inline void bench(unsigned repeat_times, T *pa)
{
	...
	wrap_op<T, OpT<T>, arr_len> op1(pa);
	uint64_t cs1 = tsc_bench(op1, warming_times, repeat_times);

	report_bench(OpT<T>::name(), repeat_times, cs1, (int)sse_vec<T>::pack_width, 1);
}
...
struct pow_op
{
	...
	LSIMD_ENSURE_INLINE
	static sse_vec<T> run(sse_vec<T> x)
	{
		return pow(x, sse_vec<T>(2.5));
	}
};
